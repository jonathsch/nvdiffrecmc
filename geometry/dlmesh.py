...
from diffusion.sd_sds import StableDiffusionSDS
...
class DLMesh:
    def __init__(self, initial_guess, FLAGS):
        ...
        print(
            "Avg edge length: %f"
            % regularizer.avg_edge_length(self.mesh.v_pos, self.mesh.t_pos_idx)
        )
    ...
    def getMesh(self, material):
        ...
        with torch.no_grad():
            ou.optix_build_bvh(
                self.optix_ctx, imesh.v_pos.contiguous(), imesh.t_pos_idx.int(), rebuild=1
            )
    ...
    def tick(self, glctx, target, lgt, opt_material, loss_fn, iteration, FLAGS, denoiser):
        ...
        buffers = render.render_mesh(
            ...
            target["mvp"],
            target["campos"],
            target["light"] if lgt is None else lgt,
            target["resolution"],
            spp=target["spp"],
        ...
        reg_loss += regularizer.shading_loss(
            buffers["diffuse_light"],
            buffers["specular_light"],
            color_ref,
            FLAGS.lambda_diffuse,
            FLAGS.lambda_specular,
        )

        # Material smoothness regularizer
        reg_loss += regularizer.material_smoothness_grad(
            buffers["kd_grad"],
            buffers["ks_grad"],
            buffers["normal_grad"],
            lambda_kd=self.FLAGS.lambda_kd,
            lambda_ks=self.FLAGS.lambda_ks,
            lambda_nrm=self.FLAGS.lambda_nrm,
        )
